...
export function LegCard({
  ...
  showAccept = true,
  onAccept,
}: { leg: Leg; showAccept?: boolean; onAccept?: (legId: string) => Promise<void> }) {
      ...
      <div className="p-5 flex flex-col gap-4">
        ...
        {showAccept && !accepted && leg.status === "OPEN" && (
          <button
            ...
            disabled={accepting}
            className="w-full rounded-2xl bg-success text-success-foreground font-bold text-base py-4 min-h-[56px] active:scale-[0.98] transition-transform disabled:opacity-70"
          >
            {accepting ? "Accepting…" : "Accept This Leg"}
          </button>
        )}
        {accepted && (
