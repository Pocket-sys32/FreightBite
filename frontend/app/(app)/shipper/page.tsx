...
import { submitLoad, getLoad, placeToCoords, type UILoad } from "@/lib/api"
...
export default function ShipperPortalPage() {
  ...
  const [load, setLoad] = useState<UILoad | null>(null)
  const [submitError, setSubmitError] = useState<string | null>(null)

  const handleSubmit = async () => {
    const originCoords = placeToCoords(origin)
    const destCoords = placeToCoords(destination)
    if (!originCoords || !destCoords) {
      setSubmitError("Use known places: Melrose Park IL, Chicago IL, Rialto CA, Los Angeles CA")
      return
    }
    setLoading(true)
    setSubmitError(null)
    try {
      const res = await submitLoad(originCoords, destCoords)
      const full = await getLoad(res.load.id)
      if (full) setLoad(full)
      setSubmitted(true)
    } catch (e) {
  ...
  const displayLoad = load ?? DEMO_LOAD
  const totalRate = displayLoad.legs.reduce((sum, leg) => sum + leg.rateCents + leg.fuelSurchargeCents, 0)
  const lineHaulOnly = displayLoad.legs.reduce((sum, leg) => sum + leg.rateCents, 0)
  const fscTotal = displayLoad.legs.reduce((sum, leg) => sum + leg.fuelSurchargeCents, 0)
  const assignedLegs = displayLoad.legs.filter(
        ...
        <>
          ...
          <div className="rounded-2xl border border-border bg-card p-5">
            <div className="flex flex-wrap items-center gap-x-6 gap-y-2 text-xs text-muted-foreground">
              <span className="font-mono text-foreground font-semibold">{displayLoad.referenceNumber}</span>
              <span>{displayLoad.commodity}</span>
              <span className="flex items-center gap-1"><Weight className="h-3 w-3" /> {(displayLoad.weight / 1000).toFixed(1)}k lbs</span>
              <span>{displayLoad.equipment}</span>
              <span>Pickup: {displayLoad.pickupDate}</span>
              <span>Delivery: {displayLoad.deliveryDate}</span>
            </div>
            <div className="flex flex-wrap items-center gap-x-6 gap-y-1 text-[10px] text-muted-foreground mt-2">
              <span>Shipper: {displayLoad.shipper}</span>
              <span>Consignee: {displayLoad.consignee}</span>
            </div>
          </div>
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-5">
            {[
              { icon: Route, value: displayLoad.miles.toLocaleString(), unit: "mi", label: "Total Miles" },
              { icon: Truck, value: String(displayLoad.legs.length), unit: "legs", label: "Relay Legs" },
              { icon: MapPin, value: `${assignedLegs}/${displayLoad.legs.length}`, unit: "", label: "Assigned" },
          ...
          <div className="grid gap-6 lg:grid-cols-[1fr_380px]">
              ...
              <div className="p-7">
                <div className="flex items-center justify-between mb-6">
                  ...
                  <div className="flex items-center gap-2 text-xs text-muted-foreground">
                    ...
                    {displayLoad.origin} {">"} {displayLoad.destination}
                  </div>
                </div>
            ...
            <div className="rounded-2xl border border-border bg-card h-fit">
              <div className="p-7">
                ...
                <RouteVisualizer legs={displayLoad.legs} />
              </div>
            </div>
          </div>
        </>
